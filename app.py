...
def load_universe():
    ...
    if frame is None or frame.empty:
        raise RuntimeError(
            "Unable to load universe: " + "; ".join(errors)
        )
    ...
    if not symbol:
        raise RuntimeError(
            f"Universe has no symbol column: {list(frame.columns)}"
        )
    ...
    if len(result) < 120:
        raise RuntimeError(
            f"Only {len(result)} stocks mapped; too many missing"
        )
...
def load_history(ids, start, end):
    ...
    progress = st.progress(
        0,
        text="Loading previous-day OHLC once for today...",
    )
...
def render():
    ...
    if market_open(now):
        try:
            st.session_state.last_live_quotes = (
                live_quotes(ids)
            )
    ...
    # FIX:
    # Sort before reindexing because Signal Priority
    # is used for sorting but is not displayed.
    display_data = (
        data.sort_values(
            [
                "Signal Priority",
    ...
    st.caption(
        "Signals are enabled only during market hours "
        "with a quote snapshot no older than 5 minutes."
    )
