...
def load_universe():
    ...
    errors = []
    try:
        request_with_retry("GET", "https://www.nseindia.com/", session=session, timeout=20)
        response = request_with_retry("GET", NSE_URL, session=session, timeout=30)
    ...
    if frame is None or frame.empty:
        try:
            response = request_with_retry("GET", CSV_URL, session=session, timeout=30)
            response.raise_for_status()
    ...
    if frame is None or frame.empty:
        raise RuntimeError("Unable to load Nifty Midcap 150 universe: " + "; ".join(errors))

    symbol_col = column_name(frame, ["symbol", "company_symbol"])
    sector_col = column_name(frame, ["industry", "sector", "industry_info"])
    if not symbol_col:
        raise RuntimeError(f"Universe has no symbol column: {list(frame.columns)}")

    frame["stock"] = frame[symbol_col].astype(str).str.upper().str.strip()
    frame = frame[~frame["stock"].str.contains("NIFTY", na=False)].copy()
    frame["sector"] = frame[sector_col].astype(str).str.strip() if sector_col else "UNKNOWN"
    frame["exact_key"] = frame["stock"]
    frame["key"] = frame["stock"].map(clean_symbol)

    response = request_with_retry("GET", MASTER_URL, session=session, timeout=90)
    response.raise_for_status()
    master = pd.read_csv(StringIO(response.text), low_memory=False)

    segment_col = column_name(master, ["exchange_segment", "exchange_segment_name", "exchange_segment_code", "sem_exm_exch_id"])
    symbol_master_col = column_name(master, ["symbol", "trading_symbol", "sem_trading_symbol"])
    sid_col = column_name(master, ["security_id", "sem_security_id", "sem_smst_security_id"])
    market_col = column_name(master, ["sem_segment", "segment"])
    if not all([segment_col, symbol_master_col, sid_col]):
        raise RuntimeError(f"Dhan master columns not recognised: {list(master.columns)[:30]}")

    dm = master.copy()
    exchange = dm[segment_col].astype(str).str.upper().str.strip()
    dm = dm[exchange.isin({"NSE", "NSE_EQ", "NSE EQUITY", "NSE_EQ_CM"})].copy()
    if market_col:
        market = dm[market_col].astype(str).str.upper().str.strip()
        dm = dm[market.isin({"E", "EQUITY", "NSE_EQ", "NSE EQUITY", "NA", "NAN", "NSE_EQ_CM"})].copy()

    dm["security_id"] = pd.to_numeric(dm[sid_col], errors="coerce")
    dm = dm.dropna(subset=["security_id"]).copy()
    dm["exact_key"] = dm[symbol_master_col].astype(str).str.upper().str.strip()
    dm["key"] = dm[symbol_master_col].map(clean_symbol)
    ...
    fallback = dm.drop_duplicates("key").set_index("key")["security_id"]
    missing = result["security_id"].isna()
    result.loc[missing, "security_id"] = result.loc[missing, "key"].map(fallback)
    ...
    if len(result) < 120:
        raise RuntimeError(f"Only {len(result)} stocks mapped; expected at least 120")
    return result


@st.cache_data(ttl=15, show_spinner=False)
def live_quotes(ids):
    response = request_with_retry("POST", QUOTE_URL, headers=dhan_headers(), json={"NSE_EQ": sorted({int(x) for x in ids})}, timeout=60)
    response.raise_for_status()
...
def previous_ohlc(sid, start, end):
    ...
    response = request_with_retry("POST", HISTORY_URL, headers=dhan_headers(), json=payload, timeout=30)
    response.raise_for_status()
...
def calculate(data, live_is_fresh):
    numeric = ["LTP", "PDC", "PDH", "PDL", "Today's Low", "Today's High"]
    ...
    count = valid.groupby(data["Sector"]).transform("sum")
    ...
    data["Green Signal"] = valid & live_is_fresh & reliable_sector & data["Today's High"].gt(data["PDH"]) & data["Today's Low"].lt(data["PDH"]) & data["LTP"].gt(data["PDH"]) & data["Sector A/D Ratio"].gt(1) & tight_up
    data["Red Signal"] = valid & live_is_fresh & reliable_sector & data["Today's Low"].lt(data["PDL"]) & data["Today's High"].gt(data["PDL"]) & data["LTP"].lt(data["PDL"]) & data["Sector A/D Ratio"].lt(1) & tight_down
    data["Signal"] = ""
...
def render():
    ...
    if quote_time:
        age = max(0, int((now - quote_time).total_seconds()))
        live_is_fresh = age <= MAX_QUOTE_AGE_SECONDS and market_open(now)
        status = "🟢 Fresh" if live_is_fresh else "⚠️ Stale"
        st.caption(f"{status} LTP snapshot: {quote_time.strftime('%Y-%m-%d %H:%M:%S %Z')} ({age}s old) • refresh target: 15s")
    if quote_error:
    ...
    data = calculate(pd.DataFrame(rows), live_is_fresh)
    ...
    display_data = data.sort_values(["Signal Priority", "Sector", "Stock"], kind="stable").reindex(columns=columns)
    st.metric("Stocks scanned", len(data))
    st.dataframe(display_data, use_container_width=True, hide_index=True)
    st.caption("LTP refreshes every 15 seconds during market hours. Signals require fresh quotes and at least 5 valid stocks in the sector.")
...
try:
    if hasattr(st, "fragment"):
        st.fragment(run_every="15s")(render)()
    else:
        render()
except Exception as exc:
